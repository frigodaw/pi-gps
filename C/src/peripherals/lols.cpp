...
Std_ReturnType Gps_ResetDataOnInit(void)   
{
   ...
   Gps_mainData.isActive = FALSE;
   Gps_mainData.fileDescriptor = 0u;
...
Std_ReturnType Gps_SerialInit(void)
{
   ...
   Gps_mainData.fileDescriptor = open(Gps_serialDirectory, O_RDONLY | O_NOCTTY);
                  /* O_RDWR   - Read only access to serial port        */
                  /* O_NOCTTY - No terminal will control the process   */
                  /* Open in blocking mode, read will wait             */
                     
   if(Gps_mainData.fileDescriptor == -1)  /* Error Checking */
   {
   ...
   tcgetattr(Gps_mainData.fileDescriptor, &Gps_serialPortSettings);	/* Get the current attributes of the Serial port */
   ...
   Gps_serialPortSettings.c_cc[VTIME] = 20;  /* Wait 2s   */

   /* Set the attributes to the termios structure */
   if((tcsetattr(Gps_mainData.fileDescriptor,TCSANOW, &Gps_serialPortSettings)) != 0) 
   {
...
{
   close(Gps_mainData.fileDescriptor); 
}
...
Std_ReturnType Gps_Main(void)
{
   ...
   if(OK == retVal)  //data available
   {
      ...
      Gps_FilterData_GPGGA(dataBuffer);
   }
}
...
Std_ReturnType Gps_GetData(char* buffer)
{
   ...
   tcflush(Gps_mainData.fileDescriptor, TCIFLUSH);                          /* Discards old data in the rx buffer */
   bytesRead = read(Gps_mainData.fileDescriptor, buffer, GPS_BUFFERSIZE);   /* Read the data                      */
