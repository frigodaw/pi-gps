...
Std_ReturnType Gps_SerialInit(void)
{
   ...
   fileDescriptor = open(Gps_serialDirectory, O_RDONLY | O_NOCTTY);
   ...
   tcgetattr(fileDescriptor, &Gps_serialPortSettings);	/* Get the current attributes of the Serial port */
   ...
   if((tcsetattr(fileDescriptor,TCSANOW, &Gps_serialPortSettings)) != 0) 
   {
...
Std_ReturnType Gps_GetData(char* buffer)
{
   ...
   tcflush(fileDescriptor, TCIFLUSH);                          /* Discards old data in the rx buffer */
   bytesRead = read(fileDescriptor, buffer, GPS_BUFFERSIZE);   /* Read the data                      */
